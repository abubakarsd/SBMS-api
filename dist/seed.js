...
const seed = async () => {
    try {
        ...
        // Seed Customers
        const customerCount = await customerModel_1.default.countDocuments();
        if (customerCount < 20) {
            const customers = [
            ...
            await customerModel_1.default.insertMany(customers);
            console.log(`${customers.length} customers seeded`);
        }
        // Seed Staff
        const staffCount = await staffModel_1.default.countDocuments();
        if (staffCount < 10) {
            const staff = [
            ...
            await staffModel_1.default.insertMany(staff);
            console.log(`${staff.length} staff members seeded`);
        }
        // Seed Sales (for last 7 days)
        const salesCount = await saleModel_1.default.countDocuments();
        if (salesCount < 20) {
            ...
            await saleModel_1.default.insertMany(sales);
